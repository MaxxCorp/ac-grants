...
<div class="tvl-container">
	...
	<!-- Tariff Compliance & Verification Banner -->
	{#if tvl.tariffValidation}
		<div class="tariff-validation-banner {tvl.tariffValidation.isCompliant ? 'val-compliant' : 'val-discrepancy'}">
			<div class="val-header">
				<div class="val-title-group">
					<span class="val-icon">{tvl.tariffValidation.isCompliant ? '🛡️' : '⚠️'}</span>
					<div>
						<div class="val-title">
							{#if tvl.tariffValidation.isCompliant}
								AWO-Tarifprüfung: Alle Ist-Entgelte sind tarifkonform
							{:else}
								AWO-Tarifprüfung: {tvl.tariffValidation.discrepancyCount} Abweichung(en) im Berechnungsblatt erkannt
							{/if}
						</div>
						<div class="val-sub">{tvl.tariffValidation.summaryText}</div>
					</div>
				</div>

				<div class="val-actions">
					{#if !tvl.tariffValidation.isCompliant}
						<button type="button" class="btn-toggle-discrepancies" onclick={() => (showDiscrepancies = !showDiscrepancies)}>
							{showDiscrepancies ? 'Details ausblenden' : 'Abweichungen anzeigen'}
						</button>
						<button type="button" class="btn-apply-tariffs" onclick={applyOfficialAwoTariffValues}>
							{#if copiedField === 'applied-official-tariffs'}
								✓ Tarifwerte angewendet!
							{:else}
								⚡ Offizielle AWO-Tarifwerte übernehmen
							{/if}
						</button>
					{:else}
						<button type="button" class="btn-apply-tariffs outline" onclick={applyOfficialAwoTariffValues}>
							{#if copiedField === 'applied-official-tariffs'}
								✓ Tarifwerte synchronisiert!
							{:else}
								🔄 Aus AWO-Tarif übernehmen
							{/if}
						</button>
					{/if}
				</div>
			...
			{#if showDiscrepancies && tvl.tariffValidation.discrepancies.length > 0}
					...
					<table class="discrepancies-table">
						<thead>
							<tr>
								<th>Monat</th>
								<th>Tarifgruppe</th>
								<th class="th-num">Berechnungsblatt (Ist)</th>
								<th class="th-num">Offizieller AWO-Tarif</th>
								<th class="th-num">Differenz</th>
							</tr>
						</thead>
						<tbody>
							{#each tvl.tariffValidation.discrepancies as d}
								<tr>
									<td class="font-mono">{d.month}/{d.year}</td>
									<td>{d.group}/{d.step}</td>
									<td class="text-right font-mono font-bold text-rose">{formatCurrency(d.recordedPartTimeSalary)}</td>
									<td class="text-right font-mono font-bold text-emerald">{formatCurrency(d.expectedPartTimeSalary)}</td>
									<td class="text-right font-mono font-bold {d.diffPartTime > 0 ? 'text-rose' : 'text-emerald'}">
										{d.diffPartTime > 0 ? '+' : ''}{formatCurrency(d.diffPartTime)}
									</td>
								</tr>
							{/each}
						</tbody>
...
<style>
	...
	.tariff-validation-banner.val-compliant {
		background: linear-gradient(135deg, rgba(16, 185, 129, 0.08) 0%, rgba(5, 150, 105, 0.03) 100%);
		border-color: rgba(16, 185, 129, 0.25);
	}

	.tariff-validation-banner.val-discrepancy {
	...
	.btn-toggle-discrepancies:hover {
		color: #ffffff;
		background: rgba(51, 65, 85, 0.9);
	}
	...
	.btn-apply-tariffs:hover {
		background: linear-gradient(135deg, #d97706 0%, #b45309 100%);
		transform: translateY(-1px);
	}

	.btn-apply-tariffs.outline {
		background: rgba(30, 41, 59, 0.8);
		color: #38bdf8;
	...
	.btn-apply-tariffs.outline:hover {
		background: rgba(56, 189, 248, 0.15);
		color: #ffffff;
	}

	.discrepancies-table-wrap {
		border: 1px solid rgba(255, 255, 255, 0.1);
		border-radius: 6px;
		overflow: hidden;
		margin-top: 0.5rem;
